...
student = (name = "Andreas Theodosiou", kerberos_id = "at694")
...
random_vect = rand(1:10, 10)  # replace this with your code!
...
# ╔═╡ bb3143ac-f924-11ea-216f-bd39f9c33bdf
function sum_to(n)
	s = 0 # new local
   for i = 1:n
...
# ╔═╡ 11e8f422-f986-11ea-1afa-79293188d743
A = [ x+y for x=1:3, y=1:4 ]

# ╔═╡ 8c19fb72-ed6c-11ea-2728-3fa9219eddc4
function vecvec_to_matrix(vecvec)
	return [ vecvec[y][x] for y=1:length(vecvec), x=1:length(vecvec[1]) ]
end
...
function matrix_to_vecvec(matrix)
	return [row for row in eachrow(matrix)]
end
...
# ╔═╡ 807d6d1e-fac6-11ea-072f-d770e5e5eddb
md"""
#### Exercise 2.1
...
# ╔═╡ abbe9b0e-fac8-11ea-1803-23f451f7fa1b
RGBX(0, 0, 0)
...
# ╔═╡ eff43f76-fb82-11ea-24de-5780bd2c6e39
...
# ╔═╡ c1e713c8-fb83-11ea-3fd9-f1b1c7d22318
a = RGB(0.2, 0.3, 0.4)
...
begin
	...
	function noisify(color::AbstractRGB, s)
		return RGB(noisify(color.r, s), noisify(color.b, s), noisify(color.b, s))
	end
...
The image is unrecognisable with intensity 4.29. As the magnitude of the noise increases beyond the range of the RGB values, each pixel if moved very far away from its original value to the point were it is unrecongisable. Thus, the image becomes unrecognisable.
"""
...
function convolve_vector(v, k)
	...
	return [extended_v[i:i+lₖ-1] ⋅ k for i ∈ 1:lₖ:length(extended_v)]
end
end
...
function blur_1D(v, l)
	#return [ mean(extend(v[i-1:i+1])) for i in 1:length(v) ]
	side = 2 * l + 1
...
# ╔═╡ 0d99f92a-fd0c-11ea-24de-e3882f0a98d3
colored_line(v)

# ╔═╡ 7d6f50fe-fd0c-11ea-0fb3-5dede91605e2
@bind l_box Slider(0:50, show_value=true)

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
blured_v = blur_1D(v, l_box)

# ╔═╡ 14a64d72-fd0c-11ea-0f0c-4593f0e779c5
...
@bind gaussian_kernel_size_1D Slider(3:1:50, show_value=true) # change this value, or turn me into a slider!
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	extended_M = [ extend_mat(M, i, j) for (i,j) ∈ Iterators.product(1-row_offset:rows+row_offset, 1-col_offset:cols+col_offset) ]
...
function with_gaussian_blur(image, l=100000)
...
# ╔═╡ def6071e-0487-11eb-1695-49d0ec2c5921
begin
Base.:^(b::ColorTypes.RGB{Float64}, p::Number) = RGB{Float64}(b.r^p, b.g^p, b.b^p)
end
...
with_gaussian_blur(gauss_camera_image, 3)

# ╔═╡ bf21980e-0487-11eb-30b4-93d1856c34ff
typeof(gauss_camera_image)

# ╔═╡ bf892188-048a-11eb-252d-f79671349602
gauss_camera_image

# ╔═╡ 25c7e366-0489-11eb-0220-ab7e62539477
gauss_camera_image + gauss_camera_image

...
double_letters = let
	alphabet[findall(!iszero, diag(sample_freq_matrix))]
end
...
# ╔═╡ d360f0ea-4c66-11eb-0593-e5227b494668
row_sum = sum(sample_freq_matrix, dims=2)

# ╔═╡ db6071e4-4c66-11eb-2f21-697b8383308b
col_sum = transpose(sum(sample_freq_matrix, dims=1))
...
# ╔═╡ 290942d4-4cf5-11eb-02ba-65f002f10d10
function word_counts_alt(words::Vector)
	Dict(k => count(==(k), words) for k in unique(words))
end

# ╔═╡ 62050714-4cf5-11eb-354d-6536493e666a
word_counts_alt(["to", "be", "or", "not", "to", "be"])
...
# ╔═╡ 17ec0e98-4d39-11eb-20fd-b9ed7f523eac
rand(ngrams(split("to be or not to be that is the question", " "), 3))

...
The optimization reduced 342 allocations, as many as the number of rows of the image. By removing the call to vcat and directly indexing the image, we avoid having to allocate a new array every time vcat is called.
"""
...
684 allocations were avoided. This is because indexing returns a copy of the array being indexed, whereas a view returns a reference to it which does not require memorey allocation. Two indexing operations are done per row, thus by replacing them with references, $2 \times 342$ are avoided.
"""
...
# ╔═╡ d6ecc1fc-331c-11eb-3db2-5d7c11f60c11
energy(img)
...
# ╔═╡ 1317772a-0bf0-11eb-2e34-9ff223508271
begin
seams = zeros(Int, size(greedy_test, 1))
seams[1] = 2
findmin(greedy_test[2, max(seams[2-1]-1, 1):min(seams[2-1]+1, length(seams))])
end
...
# ╔═╡ 6a0d6062-0f7e-11eb-2310-f9dd542c52cd
function least_energy(energies, i, j)
	rows, columns = size(energies)
	if i == rows
		return (energies[i, j], j)
	end
	next_columns = j .+ [-1, 0, 1]
	clamp!(next_columns, 1, columns)
	println("On i:$i, j:$j") 
	(min_energy, min_col), index =  findmin([
			least_energy(energies, i+1, next_columns[1]), 
			least_energy(energies, i+1, next_columns[2]), 
			least_energy(energies, i+1, next_columns[3])])
	return (min_energy + energies[i, j], next_columns[index])
end	

# ╔═╡ 8ec27ef8-f320-11ea-2573-c97b7b908cb7
## returns lowest possible sum energy at pixel (i, j), and the column to jump to in row i+1.
function least_energy_old(energies, i, j)
	# base case
	# if i == something
	...
	function recursive_least_energy(energies, i, j)
		rows, columns = size(energies)
		if i == rows
			return (energies[i, j], j)
		end
	
		next_energy, next_pixel = findmin(energies[i+1, max(j-1, 1):min(j+1, columns)])
		next_pixel -= 2
		if j == 1
			next_pixel += 1
		end
		next_pixel += j
		return (next_energy + least_energy(energies, i+1, next_pixel)[1], next_pixel)
		end
	cumulative_energy, next_column = recursive_least_energy(energies, i, j)
	return (energies[i, j] + cumulative_energy, next_column)
end

# ╔═╡ ac8927f8-1703-11eb-31e8-41cfb95fde15
function ans_least_energy(energies, i, j)
...
The algorithm considers all possible paths from the starting pixel to the bottom row of the image, computes the cost of each one and picks the minimum. This exhausts all the possibilities.

Complexity is $\mathcal{O}(3^n)$.
"""
...
function memoized_least_energy(energies, i, j, memory)
	...
	if haskey(memory, (i, j))
		...
		min_energy =  minimum([
			memoized_least_energy(energies, i+1, next_columns[1], memory), 
			memoized_least_energy(energies, i+1, next_columns[2], memory), 
			memoized_least_energy(energies, i+1, next_columns[3], memory)])
		memory[(i, j)] = min_energy + energies[i, j]
	end
	return memory[(i, j)]
end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	for row ∈ 1:m-1
		pixel = seam[row]
		next_columns = pixel .+ [-1, 0, 1]
		clamp!(next_columns, 1, n)
		
		next_pixel_index = argmin(
			[memory[(row+1, next_columns[1])], 
			memory[(row+1, next_columns[2])], 
			memory[(row+1, next_columns[3])]])
		
		seam[row+1] = next_columns[next_pixel_index]
	end
	return seam
end

# ╔═╡ 1bbe16ee-16f5-11eb-1939-a5eb66f8e1da
size(img)
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if memory[i, j] != 0
		...
		clamp!(next_columns, 1, n)
		min_energy =  minimum([
			matrix_memoized_least_energy(energies, i+1, next_columns[1], memory), 
			matrix_memoized_least_energy(energies, i+1, next_columns[2], memory), 
			matrix_memoized_least_energy(energies, i+1, next_columns[3], memory)])
		memory[i, j] = min_energy + energies[i, j]
	end
	return memory[i, j]
end
...
if shrink_dict
	dict_carved = shrink_n(img, 50, recursive_memoized_seam)
...
if shrink_matrix
	matrix_carved = shrink_n(img, 20, matrix_memoized_seam)
	md"Shrink by: $(@bind matrix_n Slider(1:20, show_value=true))"
end
...
if shrink_bottomup
	bottomup_carved = shrink_n(img, 20, seam_from_precomputed_least_energy)
	md"Shrink by: $(@bind bottomup_n Slider(1:20, show_value=true))"
end
...
if shrink_recursive
	recursive_carved = shrink_n(pika, 3, recursive_seam, show_lightning=true)
...
# ╔═╡ 2535046a-1701-11eb-2616-7b6dff7a9902
pika

# ╔═╡ a1ea0e2a-1705-11eb-126c-c5feddac4e7d
energy(pika)

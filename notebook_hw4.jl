...
# ╔═╡ 4ad79def-f52b-4b6b-88fa-581a976fc605
begin
...
The patient is certain to recover on the first day, so τ is 1 .
"""
...
function do_experiment(p, N)	
	return map(recovery_time, Iterators.repeated(p, N))
end
...
function frequencies_plot_with_mean(data)
	...
	base = bar(frequencies(data), label="Relative Frequency")
...
# ╔═╡ 103ed025-823f-4923-8e5f-55b2b694fe70
@bind p_interactive Slider(0.01:0.01:1, show_value=true)

# ╔═╡ f33f2b39-ed8a-4808-9422-1bd94718e76c
@bind N_interactive Slider(1:100:100000, show_value=true)

# ╔═╡ bb63f3cc-042f-11eb-04ff-a128aec3c378
begin
	exp_data = do_experiment(p_interactive, N_interactive)
	frequencies_plot_with_mean(exp_data)
end
...
# ╔═╡ 992aadc6-4e4e-4f9c-9091-557d2a54bd25
md"""As p varies the mean decreases tending towards 1. This makes sense because the expected value of the time to first success of a Bernoulli process is 1/p. As N increases the mean decreases and tends towards the true value of the mean. This is a realisation of the law of large numbers.
"""
...
# ╔═╡ 5c9c0050-9f2a-41b6-9c71-b6fa4658330c
md"""The distribution has an exponential decay shape of the form $A \mathrm{e}^{kx}$ We can apply  least-squares regression to the log of the distribution to obtain an estimate of $A$ and $k$.
"""

# ╔═╡ f1693d9c-a6a7-4e0d-b958-1a6a459882d7
function fit_exponential(x, y)
	w = log.(y)
	n_x = length(x)
	b = (sum(w .* x) - (sum(w) * sum(x))/n_x) / (sum(x.^2) - sum(x)^2 / n_x)
	a = sum(w) / n_x - (b / n_x * sum(x))
	return (exp(a), b)
end

# ╔═╡ b8a2f491-fc02-4ad1-a762-7d36643ce844
begin
	sorted_frequencies = sort(frequencies(exp_data))
	xs = keys(sorted_frequencies) |> collect
	ys = values(sorted_frequencies)
	A, k = fit_exponential(xs, ys)
	bar_plot = frequencies_plot_with_mean(exp_data)
	#xs = pushfirst!(xs, 0)
	plot!(bar_plot, xs, A*exp.(k*xs), label="Fit")
end

# ╔═╡ 7bb8e426-0495-11eb-3a8b-cbbab61a1631
sort(frequencies(exp_data))
...
# ╔═╡ 321ac3d6-b40a-49d0-bbb5-5f1a9f6cd6dc
function my_mean(x)
	sum(x) / length(x)
end

# ╔═╡ 3ea098eb-3dd2-4e1e-be87-8aad8ecc5dfb
begin
	ps = 0.001:0.001:1
	mean_time = map(ps) do p
		data = do_experiment(p, 10_000)
		my_mean(data)
	end
	plot(ps, mean_time, title="Mean time to recover", xlabel="Probability", ylabel="Time / day")
end
...
# ╔═╡ cc8a6234-27e0-49c7-af1a-881d98a85ec6
Integer(test_status)
...
# ╔═╡ b329070b-f735-47c7-84fa-d0f60dbdd666
md"""It has 3 methods"""
...
# ╔═╡ 26d27f54-bb4f-45e2-9e67-6f54621c3f18
...
function generate_agents(N::Integer)
	...
	infectious_i = rand(1:N)
	set_status!(agents[infectious_i], I)
	return agents
end
...
md"""
1. The mean number of agents a single infected agent infects
2. The mean time to recover
3. The peak number of infected agents
"""
...
let
	agent = Agent(S, 0)
	source = Agent(I, 0)
	infection = InfectionRecovery(0.2, 0.6)
